...
public class AccessibilityPreferencesFragment extends PreferenceFragment
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		SettingManager settings = SettingManager.getSettingManager();
		mFormat = NumberFormat.getPercentInstance();

		Preference e = findPreference(SettingManager.PREF_MIN_FONT_SIZE);
		e.setOnPreferenceChangeListener(this);
		updateMinFontSummary(e, settings.getMinimumFontSize());
		e = findPreference(SettingManager.PREF_TEXT_ZOOM);
		e.setOnPreferenceChangeListener(this);
		updateTextZoomSummary(e, settings.getTextZoom());
		e = findPreference(SettingManager.PREF_DOUBLE_TAP_ZOOM);
		e.setOnPreferenceChangeListener(this);
		updateDoubleTapZoomSummary(e, settings.getDoubleTapZoom());
		// e = findPreference(SettingManager.PREF_INVERTED_CONTRAST);
		// e.setOnPreferenceChangeListener(this);
		// updateInvertedContrastSummary(e, (int)
		// (settings.getInvertedContrast() * 100));
	}
	...
	void updateMinFontSummary(Preference pref, int minFontSize) {
		...
		pref.setSummary(String.format(c.getString(R.string.pref_min_font_size_value), minFontSize));
	}
	...
	public boolean onPreferenceChange(Preference pref, Object objValue) {
		...
		if (SettingManager.PREF_MIN_FONT_SIZE.equals(pref.getKey())) {
			updateMinFontSummary(pref, SettingManager
					.getAdjustedMinimumFontSize((Integer) objValue));
		}
		if (SettingManager.PREF_TEXT_ZOOM.equals(pref.getKey())) {
			SettingManager settings = SettingManager.getSettingManager();
			updateTextZoomSummary(pref, settings
					.getAdjustedTextZoom((Integer) objValue));
		}
		if (SettingManager.PREF_DOUBLE_TAP_ZOOM.equals(pref.getKey())) {
			SettingManager settings = SettingManager.getSettingManager();
			updateDoubleTapZoomSummary(pref, settings
					.getAdjustedDoubleTapZoom((Integer) objValue));
		}
		if (SettingManager.PREF_INVERTED_CONTRAST.equals(pref.getKey())) {
